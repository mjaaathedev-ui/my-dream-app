...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (action === "callback") {
      ...
      if (error) {
        return Response.redirect(`${url.origin}/settings?google=error&reason=${error}`, 302);
      }
      ...
      if (!tokenRes.ok) {
        ...
        return Response.redirect(`${url.origin}/settings?google=error&reason=token_exchange`, 302);
      }
