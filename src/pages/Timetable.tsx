...
export default function Timetable() {
  ...
  const openAdd = () => { setForm({ ...blankForm }); setEditEntry(null); setShowDialog(true); };

  const openEdit = (entry: TimetableEntry) => {
    setForm({
      title: entry.title, type: entry.type, entry_type: entry.entry_type ?? "recurring",
      specific_date: entry.specific_date ?? "", day_of_week: String(entry.day_of_week),
      recurrence: entry.recurrence ?? "weekly", start_time: entry.start_time, end_time: entry.end_time,
  ...
  const saveEntry = async () => {
    ...
    const effectiveDow = form.entry_type === "once" && form.specific_date
      ? dateStringToDow(form.specific_date) : Number(form.day_of_week);

    const payload: Partial<TimetableEntry> = {
      id: editEntry?.id, user_id: user.id, title: form.title.trim(), type: form.type,
      entry_type: form.entry_type, specific_date: form.entry_type === "once" ? form.specific_date || null : null,
      day_of_week: effectiveDow, recurrence: form.entry_type === "recurring" ? form.recurrence : "weekly",
    ...
    if (conflict.hasConflict) { toast.error(conflict.message); return; }

    if (editEntry) {
      const { data, error } = await supabase.from("timetable_entries").update(payload).eq("id", editEntry.id).select().single();
  ...
  const sendAiMessage = useCallback(async () => {
    ...
    try {
      ...
      if (contentType.includes("text/event-stream") && resp.body) {
        ...
        while (true) {
          ...
          while ((nl = buf.indexOf("\n")) !== -1) {
            ...
            if (json === "[DONE]") break;
            try { rawText += JSON.parse(json).choices?.[0]?.delta?.content ?? ""; } catch {}
          }
        }
      } else {
      ...
      setAiChat((prev) => [...prev, { role: "assistant", content: parsed.message }]);
    } catch (err: any) {
      toast.error(err.message ?? "AI error");
      setAiChat((prev) => [...prev, { role: "assistant", content: "Something went wrong — please try again." }]);
    } finally {
      setAiLoading(false);
    }
  }, [aiInput, aiLoading, aiChat, entries, modules, user, profile]);

  const applyAiAction = async (resp: TimetableAIResponse) => {
    if (!user || resp.error) return;
    if (resp.action === "add" && resp.entry) await addEntryFromAI(resp.entry);
    ...
    if (resp.action === "delete_multiple" && resp.ids) {
      for (const id of resp.ids) await supabase.from("timetable_entries").delete().eq("id", id);
  ...
  const TimeGrid = ({ dates }: { dates: Date[] }) => (
    <div className="flex overflow-auto flex-1">
      ...
      {dates.map((date, di) => {
        const dayEntries = getEntriesForDate(entries, date).sort((a, b) => a.start_time.localeCompare(b.start_time));
        const todayBg = isToday(date) ? "bg-primary/5" : "";
        return (
          <div key={di} className={`flex-1 min-w-[90px] border-r border-border last:border-r-0 ${todayBg}`}>
            <div className={`h-12 border-b border-border flex flex-col items-center justify-center sticky top-0 z-10 ${isToday(date) ? "bg-primary/10" : "bg-background"}`}>
              <span className="text-[10px] text-muted-foreground font-medium uppercase tracking-wide">{format(date, "EEE")}</span>
              <span className={`text-sm font-bold leading-none ${isToday(date) ? "text-primary" : ""}`}>{format(date, "d")}</span>
            </div>
            <div className="relative" style={{ height: HOURS.length * PX_PER_HOUR }}>
              {HOURS.map((h) => (
                <div key={h} className="absolute w-full border-b border-border/40" style={{ top: (h - 6) * PX_PER_HOUR, height: PX_PER_HOUR }} />
              ))}
      ...
      <div className="flex-1 overflow-auto">
        ...
        <div className="grid grid-cols-7">
          {allDays.map((day, i) => {
            ...
            return (
              <div key={i} className={`min-h-[110px] border-b border-r border-border last:border-r-0 p-1.5 ${!inMonth ? "bg-muted/20" : ""} ${isToday(day) ? "bg-primary/5" : ""}`}>
                  ...
                  {dayEntries.slice(0, 3).map((entry) => {
                    ...
                    return (
                      <div key={entry.id} onClick={() => openEdit(entry)}
                        ...
                        {entry.entry_type === "once" && <span className="shrink-0 font-bold">1×</span>}
                        {entry.start_time} {entry.title}
                      </div>
                    );
                  })}
                  {dayEntries.length > 3 && <p className="text-[10px] text-muted-foreground pl-1">+{dayEntries.length - 3} more</p>}
                </div>
              </div>
            );
          })}
  ...
  const ListView = () => {
    ...
    return (
      <div className="flex-1 overflow-auto p-4 space-y-2">
        {sorted.length === 0 ? (
          <div className="text-center py-16 text-sm text-muted-foreground">No entries yet. Click "Add entry" or ask the AI assistant.</div>
        ) : sorted.map((entry) => {
          const color = entry.color || TIMETABLE_ENTRY_COLORS[entry.type] || "#2563EB";
            ...
            <div key={entry.id} onClick={() => openEdit(entry)} className="flex items-center gap-3 p-3 rounded-lg border border-border hover:bg-accent transition-colors cursor-pointer group">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-muted-foreground flex items-center gap-2 flex-wrap">
                  ...
                  {entry.location && <><MapPin className="h-3 w-3" /><span>{entry.location}</span></>}
        ...
        <div className="flex items-center gap-2">
          ...
          {viewMode !== "list" && (
            <div className="flex items-center gap-1">
              ...
              <Button variant="outline" size="sm" className="text-xs h-8" onClick={() => setCurrentDate(new Date())}>Today</Button>
              <Button variant="outline" size="icon" className="h-8 w-8" onClick={() => navigate(1)}><ChevronRight className="h-4 w-4" /></Button>
            </div>
          )}
          {viewMode !== "list" && <span className="text-sm font-medium hidden md:block">{headerLabel}</span>}
        </div>
        <div className="flex items-center gap-2">
          ...
          <Button size="sm" className="gap-1.5 h-8 text-xs" onClick={openAdd}><Plus className="h-3 w-3" /> Add entry</Button>
        </div>
      </div>
      ...
      <Dialog open={showDialog} onOpenChange={setShowDialog}>
        <DialogContent className="max-w-lg max-h-[90vh] overflow-auto">
          ...
          <div className="space-y-4 pt-1">
            <div className="space-y-1.5"><Label>Title *</Label><Input value={form.title} onChange={(e) => setForm((p) => ({ ...p, title: e.target.value }))} placeholder="e.g. CS201 Lecture" /></div>
            <div className="space-y-1.5">
              <Label>Entry Type</Label>
              <div className="flex rounded-md border border-border overflow-hidden">
                {(["once", "recurring"] as const).map((t) => (
                  <button key={t} type="button" onClick={() => setForm((p) => ({ ...p, entry_type: t, recurring: t === "recurring" }))}
                    className={`flex-1 py-2 text-sm transition-colors ${form.entry_type === t ? "bg-primary text-primary-foreground font-medium" : "hover:bg-accent"}`}>
            ...
            {form.entry_type === "once" ? (
              <div className="space-y-1.5"><Label>Date *</Label><Input type="date" value={form.specific_date} onChange={(e) => setForm((p) => ({ ...p, specific_date: e.target.value }))} /></div>
            ) : (
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1.5"><Label>Day *</Label>
                  <Select value={form.day_of_week} onValueChange={(v) => setForm((p) => ({ ...p, day_of_week: v }))}>
                    ...
                    <SelectContent>{DAY_NAMES_FULL.map((d, i) => <SelectItem key={i} value={String(i)}>{d}</SelectItem>)}</SelectContent>
                  </Select>
                </div>
                <div className="space-y-1.5"><Label>Repeat</Label>
                  <Select value={form.recurrence} onValueChange={(v) => setForm((p) => ({ ...p, recurrence: v as any }))}>
                    <SelectTrigger><SelectValue /></SelectTrigger>
                    <SelectContent>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1.5"><Label>Start time *</Label><Input type="time" value={form.start_time} onChange={(e) => setForm((p) => ({ ...p, start_time: e.target.value }))} /></div>
              <div className="space-y-1.5"><Label>End time *</Label><Input type="time" value={form.end_time} onChange={(e) => setForm((p) => ({ ...p, end_time: e.target.value }))} /></div>
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1.5"><Label>Type</Label>
                <Select value={form.type} onValueChange={(v) => setForm((p) => ({ ...p, type: v, color: TIMETABLE_ENTRY_COLORS[v] ?? p.color }))}>
                  <SelectTrigger><SelectValue /></SelectTrigger>
                  <SelectContent>{TIMETABLE_ENTRY_TYPES.map((t) => <SelectItem key={t} value={t} className="capitalize">{t}</SelectItem>)}</SelectContent>
                </Select>
              </div>
              <div className="space-y-1.5"><Label>Category</Label>
                <Select value={form.category} onValueChange={(v) => setForm((p) => ({ ...p, category: v }))}>
                  <SelectTrigger><SelectValue /></SelectTrigger>
                  <SelectContent>{["Lecture", "Tutorial", "Practical", "Study", "Assignment", "Exam", "Personal", "Other"].map((c) => <SelectItem key={c} value={c}>{c}</SelectItem>)}</SelectContent>
                </Select>
              </div>
            </div>
            <div className="space-y-1.5"><Label>Module</Label>
              <Select value={form.module_id || "__none__"} onValueChange={(v) => setForm((p) => ({ ...p, module_id: v === "__none__" ? "" : v }))}>
                <SelectTrigger><SelectValue placeholder="None" /></SelectTrigger>
                <SelectContent>
                  <SelectItem value="__none__">None</SelectItem>
                  {modules.map((m) => (
                    <SelectItem key={m.id} value={m.id}>
                      <div className="flex items-center gap-2"><div className="h-2 w-2 rounded-full" style={{ backgroundColor: m.color }} />{m.name}</div>
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
              ...
              <div className="flex items-center gap-2 flex-wrap">
                {Object.values(TIMETABLE_ENTRY_COLORS).map((c) => (
                  <button key={c} type="button" onClick={() => setForm((p) => ({ ...p, color: c }))}
                    className={`h-7 w-7 rounded-full border-2 transition-transform ${form.color === c ? "scale-110 border-foreground" : "border-transparent hover:scale-105"}`}
                    style={{ backgroundColor: c }} />
                ))}
                <input type="color" value={form.color} onChange={(e) => setForm((p) => ({ ...p, color: e.target.value }))} className="h-7 w-7 rounded-full cursor-pointer border-0 bg-transparent" title="Custom colour" />
              </div>
            </div>
            <div className="flex gap-2 pt-1">
              {editEntry && <Button variant="destructive" size="sm" className="gap-1.5" onClick={() => deleteEntry(editEntry.id)}><Trash2 className="h-3.5 w-3.5" /> Delete</Button>}
              <Button className="flex-1" onClick={saveEntry} disabled={!form.title.trim() || (form.entry_type === "once" && !form.specific_date)}>
