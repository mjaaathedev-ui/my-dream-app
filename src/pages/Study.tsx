...
export default function Study() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      supabase.from('goals').select('*').eq('user_id', user.id).eq('achieved', false).order('created_at', { ascending: false }),
    ]).then(([mRes, sRes, gRes]) => {
  ...
  useEffect(() => {
    ...
    intervalRef.current = window.setInterval(() => {
      ...
      if (startedAtRef.current) {
        const focused = Math.round((Date.now() - startedAtRef.current.getTime() - pauseOffsetRef.current) / 60000);
  ...
  function computeFocusedMinutes(): number {
    ...
    return Math.max(1, Math.round((Date.now() - startedAtRef.current.getTime() - pauseMs) / 60000));
  }
  ...
  async function handleTimerComplete() {
    ...
    if (user && activeSessionIdRef.current) {
      ...
      if (!error && data) {
        setSessions(prev => prev.map(s => s.id === activeSessionIdRef.current ? (data as StudySession) : s));
      }
    }
    setPhase('post');
    toast.success(`Session complete! ${duration} minutes logged.`);
  }
  ...
  const startSession = async () => {
    ...
    let minutes = sessionType === 'custom' 
      ? Math.max(1, parseInt(customMinutes, 10) || 60) 
      : (SESSION_TYPES.find(t => t.value === sessionType)?.work ?? 50);
  ...
  const skipReflection = () => resetToSetup();
    ...
    <div className="p-6 max-w-[800px] mx-auto animate-fade-in">
      <Tabs defaultValue="session">
          ...
          {phase === 'setup' && (
            <Card className="border-border shadow-sm">
              <CardContent className="p-6 space-y-5">
                ...
                {goals.length > 0 && (
                  <div className="space-y-2">
                    <Label>Linked goal <span className="text-muted-foreground font-normal text-xs">(optional)</span></Label>
                    ...
                    {energyEmojis.map((e, i) => (
                      <button key={i} onClick={() => setEnergyLevel(i + 1)}
                        className={`h-10 w-10 rounded-md text-lg border-2 transition-all
                          ${energyLevel === i + 1 ? 'border-primary bg-primary/10 scale-110' : 'border-border'}`}>
                        {e}
                      </button>
                    ))}
                  </div>
                ...
                <div className="space-y-2">
                    ...
                    {SESSION_TYPES.filter(t => t.value !== 'custom').map(t => (
                      <button key={t.value} onClick={() => setSessionType(t.value)}
                        className={`p-3 rounded-md border text-left text-sm transition-all
                          ${sessionType === t.value ? 'border-primary bg-primary/5' : 'border-border hover:bg-accent'}`}>
                        <span className="font-medium">{t.label}</span>
                      </button>
                    ))}
                    <button onClick={() => setSessionType('custom')}
                      className={`p-3 rounded-md border text-left text-sm transition-all col-span-2
                        ${sessionType === 'custom' ? 'border-primary bg-primary/5' : 'border-border hover:bg-accent'}`}>
                  ...
                  {sessionType === 'custom' && (
                    <div className="flex items-center gap-3 pt-1">
                      <Input type="number" min="1" max="480" value={customMinutes} onChange={e => setCustomMinutes(e.target.value)}
                        className="w-28 font-mono" placeholder="60" />
              ...
              <CardContent className="p-6 space-y-5">
                <div className="text-center mb-2">
                  ...
                  <p className="text-sm text-muted-foreground mt-1">
                    <span className="font-mono font-semibold text-foreground">{displayDuration} min</span> of focused study logged
                    {distractions > 0 && ` · ${distractions} distraction${distractions !== 1 ? 's' : ''}`}
                  </p>
                  {selectedGoalId && selectedGoalId !== 'none' && (
                    <p className="text-xs text-primary mt-1">🎯 {goals.find(g => g.id === selectedGoalId)?.title}</p>
                  )}
                </div>
                  ...
                  <div className="flex gap-2">
                    {energyEmojis.map((e, i) => (
                      <button key={i} onClick={() => setEnergyAfter(i + 1)}
                        className={`h-10 w-10 rounded-md text-lg border-2 transition-all
                          ${energyAfter === i + 1 ? 'border-primary bg-primary/10 scale-110' : 'border-border'}`}>
                        {e}
                      </button>
                    ))}
                  </div>
                ...
                <div className="flex gap-3">
                  <Button className="flex-1" onClick={saveReflection}>Save reflection &amp; finish</Button>
                  <Button variant="outline" onClick={skipReflection}>Skip</Button>
                </div>
              </CardContent>
            </Card>
          )}
        ...
        <TabsContent value="history">
          <div className="space-y-2">
            {sessions.length === 0 ? (
              <div className="text-center py-12 text-sm text-muted-foreground">No study sessions yet.</div>
            ) : sessions.map(s => {
              ...
              return (
                <div key={s.id} className="p-3 rounded-lg border border-border hover:bg-accent transition-colors">
                  ...
                  <div className="flex items-center gap-3 mt-2 text-xs text-muted-foreground">
                    <span className="font-mono font-medium text-foreground">{s.duration_minutes}min</span>
                    <span>{energyEmojis[(s.energy_level || 3) - 1]}</span>
                    {s.distractions_count > 0 && <span>{s.distractions_count} distraction{s.distractions_count !== 1 ? 's' : ''}</span>}
                  </div>
                  {s.reflection && <p className="text-xs text-muted-foreground mt-1.5 italic line-clamp-2">"{s.reflection}"</p>}
                </div>
              );
            })}
          </div>
